...
class TaigaDeploy(MarketPlaceAppsChatflow):
    ...
    def reservation(self):
        ...
        if not success:
            raise DeploymentFailed(
                f"Failed to deploy workload {self.workload_ids[-1]}. Your paid resources will be resued for your next deployment.",
                solution_uuid=self.solution_id,
        ...
        if not success:
            raise DeploymentFailed(
                f"Failed to create TRC container on node {self.selected_node.node_id} {self.workload_ids[-1]}. Your paid resources will be resued for your next deployment.",
                solution_uuid=self.solution_id,
                wid=self.workload_ids[-1],
            )
