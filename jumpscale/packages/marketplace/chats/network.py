...
class NetworkDeploy(BaseNetworkDeploy, MarketPlaceChatflow):
    ...
    def welcome(self):
        self.current_user = self.user_info()["username"]
        self._validate_user()
        if deployer.list_networks(self.current_user):
        ...
        self.solution_metadata["owner"] = self.current_user
    ...
    def start(self):
        if self.action == "Create":
            ...
            while not valid:
                ...
                network_solutions = solutions.list_network_solutions(self.current_user, sync=False)
                valid = True
                for sol in network_solutions:
                    if sol["Name"] == self.solution_name:
                        ...
                        break
                    valid = True
            self.solution_name = f"{self.current_user}_{self.solution_name}"
        elif self.action == "Add Access":
            self.network_view = deployer.select_network(self.current_user, self)
    ...
    def ip_config(self):
        ...
        pools = deployer.list_user_pools(self.current_user)
