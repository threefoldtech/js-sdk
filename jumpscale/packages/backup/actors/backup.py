...
import time
import os
MARKETPLACE_URL = os.environ.get('MARKETPLACE_URL', "https://marketplace.threefold.me/")
...
class Backup(BaseActor):
    ...
    def init(self, password) -> str:
        ...
        if response.status_code != 200:
            raise Exception(f"can not create user with name:{username}, error={response.text}")
    ...
    def backup(self, tags = None) -> str:
        if not self.repos_exist():
            raise Exception("repos are not inited, please call init first")
        tags = tags or []
    ...
    def get_last_snapshot(self, tags=None):
        def _add_snapshot_timestamp(snapshot):
            import arrow
            snapshot["time"] = arrow.get(snapshot["time"]).timestamp
            return snapshot
        ...
        if not snapshots:
            raise Exception("can not find any snapshots in the repo")
