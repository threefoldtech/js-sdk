...
class FundPricesDifference(BackgroundService):
    ...
    def job(self):
        j.logger.info("Fund prices difference service")
        initialization_wallet_name = j.core.config.get("VDC_INITIALIZATION_WALLET")
        if not initialization_wallet_name:
            j.logger.critical("No initialization wallet configured on the VDC deployer")
            return
        ...
        for vdc_name in VDCFACTORY.list_all():
            ...
            if vdc_instance.is_blocked or vdc_instance.is_empty():
                j.logger.info(f"VDC {vdc_instance.instance_name} is empty or in grace period")
                return
            ...
            if prepaid_balance < vdc_spec_price + TRANSACTION_FEES:
                j.logger.info(f"VDC {vdc_instance.instance_name} hove not enough money for renew plan")
                return
            elif prepaid_balance > vdc_spec_price:
                # TODO: transfer TRANSACTION FEES to the prepaid wallet
                j.logger.info(
                    f"VDC {vdc_instance.instance_name} prepaid wallet have the hour cost but don't have the transaction fees {TRANSACTION_FEES} TFT"
                )
                return
            ...
            if diff > 0:
                ...
                j.logger.info(f"VDC {vdc_instance.instance_name} funded with {diff} TFT")
