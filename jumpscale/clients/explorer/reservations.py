...
from .models import Reservation, PaymentDetail
...
class Reservations(BaseResource):
    ...
    def create(self, reservation):
        ...
        return PaymentDetail.from_dict(resp.json())
    ...
    def list(self, customer_tid=None, next_action=None, page=None):
        if page:
            ...
            reservations, _ = get_page(self._session, page, Reservation, self._url, query)
        else:
    ...
    def iter(
        ...
        if next_action:
            query["next_action"] = self._next_action(next_action)
        yield from get_all(self._session, Reservation, self._url, query)
