...
class NetworkView:
    class dry_run_context(ContextDecorator):
        ...
        def __exit__(self, *exc):
            ...
            for workload in network_view.network_workloads:
                j.sals.zos.get_zos_for().workloads.decomission(workload.id)

    def __init__(self, name, workloads=None, nodes=None):
        self.name = name
        if not workloads:
            workloads = j.sals.zos.get_zos_for().workloads.list(j.core.identity.me.tid, NextAction.DEPLOY)
        self.workloads = workloads
        self.used_ips = []
        self.network_workloads = []
        nodes = nodes or {node.node_id for node in j.sals.zos.get_zos_for()._explorer.nodes.list()}
    ...
    def add_node(self, node, pool_id):
        ...
        for workload in self.network_workloads:
            ...
            network = j.sals.zos.get_zos_for().network.create(self.iprange, self.name)
            node_workloads = {}
            ...
            j.sals.zos.get_zos_for().network.add_node(network, node.node_id, str(subnet), pool_id)
            return network
    ...
    def add_access(self, node_id=None, use_ipv4=True, pool_id=None):
        ...
        if node_id not in node_workloads:
            j.sals.zos.get_zos_for().network.add_node(network, node_id, str(subnet), pool_id=pool_id)
        wg_quick = j.sals.zos.get_zos_for().network.add_access(network, node_id, str(subnet), ipv4=use_ipv4)
    ...
    def delete_access(self, ip_range, node_id=None):
        ...
        network = j.sals.zos.get_zos_for().network.delete_access(network, node_id, ip_range)
        return network
    ...
    def dry_run(self, test_network_name=None, node_ids=None, pool_ids=None, bot=None, breaking_node_ids=None):
        ...
        network = j.sals.zos.get_zos_for().network.create(str(ip_range), name)
        for idx, subnet in enumerate(ip_range.subnet(24)):
            if idx == len(node_ids):
                break
            j.sals.zos.get_zos_for().network.add_node(
        ...
        for resource in network.network_resources:
            ...
            try:
                result.append(j.sals.zos.get_zos_for().workloads.deploy(resource))
...
class ChatflowDeployer:
    ...
    def load_user_workloads(self, next_action=NextAction.DEPLOY):
        all_workloads = j.sals.zos.get_zos_for().workloads.list(j.core.identity.me.tid, next_action)
    ...
    def list_networks(self, next_action=NextAction.DEPLOY, sync=True):
        ...
        network_views = {}
        nodes = {node.node_id for node in j.sals.zos.get_zos_for()._explorer.nodes.list()}
...
""",
    ...
    def create_pool(self, bot):
        ...
        try:
            pool_info = j.sals.zos.get_zos_for().pools.create(cu, su, farm, currencies)
        except Exception as e:
    ...
    def extend_pool(self, bot, pool_id):
        ...
        currencies = ["TFT"]
        try:
            pool_info = j.sals.zos.get_zos_for().pools.extend(pool_id, cu, su, currencies=currencies)
        except Exception as e:
            raise StopChatFlow(f"failed to extend pool.\n{str(e)}")
        qr_code = self.show_payment(pool_info, bot)
        pool = j.sals.zos.get_zos_for().pools.get(pool_id)
    ...
    def check_farm_capacity(self, farm_name, currencies=None, sru=None, cru=None, mru=None, hru=None, ip_version=None):
        ...
        if ip_version and ip_version not in ["IPv4", "IPv6"]:
            ...
            if ip_version == "IPv4":
                node_filter = j.sals.zos.get_zos_for().nodes_finder.filter_public_ip4
            elif ip_version == "IPv6":
                node_filter = j.sals.zos.get_zos_for().nodes_finder.filter_public_ip6
        currencies = currencies or []
        farm_nodes = j.sals.zos.get_zos_for().nodes_finder.nodes_search(farm_name=farm_name)
        available_cru = 0
        available_sru = 0
        available_mru = 0
        available_hru = 0
        ...
        for node in farm_nodes:
            if "FreeTFT" in currencies and not node.free_to_use:
                continue
            if not j.sals.zos.get_zos_for().nodes_finder.filter_is_up(node):
                continue
    ...
    def select_pool(
        ...
        for pool in available_pools:
            nodes = j.sals.zos.get_zos_for().nodes_finder.nodes_by_capacity(
                pool_id=pool, sru=sru, mru=mru, hru=hru, cru=cru
            )
            if not nodes:
                continue
    ...
    def get_pool_farm_id(self, pool_id=None, pool=None):
        pool = pool or j.sals.zos.get_zos_for().pools.get(pool_id)
    ...
    def deploy_network(self, name, access_node, ip_range, ip_version, pool_id, **metadata):
        network = j.sals.zos.get_zos_for().network.create(ip_range, name)
        node_subnets = netaddr.IPNetwork(ip_range).subnet(24)
        network_config = dict()
        use_ipv4 = ip_version == "IPv4"

        j.sals.zos.get_zos_for().network.add_node(network, access_node.node_id, str(next(node_subnets)), pool_id)
        wg_quick = j.sals.zos.get_zos_for().network.add_access(
        ...
        for workload in network.network_resources:
            ...
            ids.append(j.sals.zos.get_zos_for().workloads.deploy(workload))
            parent_id = ids[-1]
    ...
    def add_access(
        ...
        for resource in node_workloads.values():
            ...
            result["ids"].append(j.sals.zos.get_zos_for().workloads.deploy(resource))
    ...
    def wait_workload(self, workload_id, bot=None, expiry=10, breaking_node_id=None):
        ...
        workload = j.sals.zos.get_zos_for().workloads.get(workload_id)
    ...
    def deploy_volume(self, pool_id, node_id, size, volume_type=DiskType.SSD, **metadata):
        volume = j.sals.zos.get_zos_for().volume.create(node_id, pool_id, size, volume_type)
        if metadata:
            volume.info.metadata = self.encrypt_metadata(metadata)
        return j.sals.zos.get_zos_for().workloads.deploy(volume)
        ...
        **metadata,
        ...
        if secret_env:
            for key, val in secret_env.items():
                encrypted_secret_env[key] = j.sals.zos.get_zos_for().container.encrypt_secret(node_id, val)
        container = j.sals.zos.get_zos_for().container.create(
            node_id,
            network_name,
            ip_address,
            flist,
        ...
        if volumes:
            for mount_point, vol_id in volumes.items():
                j.sals.zos.get_zos_for().volume.attach_existing(container, f"{vol_id}-1", mount_point)
        if metadata:
            container.info.metadata = self.encrypt_metadata(metadata)
        if log_config:
            j.sals.zos.get_zos_for().container.add_logs(container, **log_config)
        return j.sals.zos.get_zos_for().workloads.deploy(container)
    ...
    def ask_container_placement(
        ...
        if j.config.get("OVER_PROVISIONING"):
            ...
            mru = 0
        nodes = j.sals.zos.get_zos_for().nodes_finder.nodes_by_capacity(
            pool_id=pool_id, cru=cru, sru=sru, mru=mru, hru=hru
        )
        nodes = list(nodes)
    ...
    def delegate_domain(self, pool_id, gateway_id, domain_name, **metadata):
        domain_delegate = j.sals.zos.get_zos_for().gateway.delegate_domain(gateway_id, domain_name, pool_id)
        if metadata:
            domain_delegate.info.metadata = self.encrypt_metadata(metadata)
        return j.sals.zos.get_zos_for().workloads.deploy(domain_delegate)

    def deploy_kubernetes_master(
        self, pool_id, node_id, network_name, cluster_secret, ssh_keys, ip_address, size=1, **metadata
    ):
        master = j.sals.zos.get_zos_for().kubernetes.add_master(
        ...
        return j.sals.zos.get_zos_for().workloads.deploy(master)
    ...
    def deploy_kubernetes_worker(
        ...
        worker = j.sals.zos.get_zos_for().kubernetes.add_worker(
        ...
        return j.sals.zos.get_zos_for().workloads.deploy(worker)
    ...
    def list_all_gateways(self, pool_ids=None):
        ...
        """
        all_gateways = filter(j.sals.zos.get_zos_for().nodes_finder.filter_is_up, self._explorer.gateway.list())
        if not all_gateways:
    ...
    def create_ipv6_gateway(self, gateway_id, pool_id, public_key, **metadata):
        ...
        workload = j.sals.zos.get_zos_for().gateway.gateway_4to6(gateway_id, public_key, pool_id)
        if metadata:
            workload.info.metadata = self.encrypt_metadata(metadata)
        return j.sals.zos.get_zos_for().workloads.deploy(workload)

    def deploy_zdb(self, pool_id, node_id, size, mode, password, disk_type="SSD", public=False, **metadata):
        workload = j.sals.zos.get_zos_for().zdb.create(node_id, size, mode, password, pool_id, disk_type, public)
        if metadata:
            workload.info.metadata = self.encrypt_metadata(metadata)
        return j.sals.zos.get_zos_for().workloads.deploy(workload)
    ...
    def create_subdomain(self, pool_id, gateway_id, subdomain, addresses=None, **metadata):
        ...
        workload = j.sals.zos.get_zos_for().gateway.sub_domain(gateway_id, subdomain, addresses, pool_id)
        if metadata:
            workload.info.metadata = self.encrypt_metadata(metadata)
        return j.sals.zos.get_zos_for().workloads.deploy(workload)
    ...
    def create_proxy(self, pool_id, gateway_id, domain_name, trc_secret, **metadata):
        ...
        """
        workload = j.sals.zos.get_zos_for().gateway.tcp_proxy_reverse(gateway_id, domain_name, trc_secret, pool_id)
        if metadata:
            workload.info.metadata = self.encrypt_metadata(metadata)
        return j.sals.zos.get_zos_for().workloads.deploy(workload)
    ...
    def get_zdb_url(self, zdb_id, password):
        workload = j.sals.zos.get_zos_for().workloads.get(zdb_id)
