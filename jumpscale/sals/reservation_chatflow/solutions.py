...
class ChatflowSolutions:
    ...
    def list_network_solutions(self, next_action=NextAction.DEPLOY, sync=True):
        ...
        farms = {}
        if networks:
            nodes = {node.node_id: node.farm_id for node in j.sals.zos.get_zos_for()._explorer.nodes.list()}
            farms = {farm.id: farm.name for farm in j.sals.zos.get_zos_for()._explorer.farms.list()}
    ...
    def cancel_solution(self, solution_wids):
        ...
        """
        workload = j.sals.zos.get_zos_for().workloads.get(solution_wids[0])
        solution_uuid = self.get_solution_uuid(workload)
        ids_to_delete = []
        if solution_uuid:
            # solutions created by new chatflows
            for workload in j.sals.zos.get_zos_for().workloads.list(j.core.identity.me.tid, next_action="DEPLOY"):
        ...
        for wid in ids_to_delete:
            j.sals.zos.get_zos_for().workloads.decomission(wid)
        ...
        for name in container_workloads:
            ...
            for c_dict in container_workloads[name]:
                ...
                if (proxy_type and proxy_type not in c_dict["flist"]) or not proxy_type:
                    pool = j.sals.zos.get_zos_for().pools.get(c_dict["pool"])
                    solution_dict.update(
                        {
    ...
    def get_workloads_by_uuid(self, solution_uuid, next_action=None):
        workloads = []
        for workload in j.sals.zos.get_zos_for().workloads.list(j.core.identity.me.tid, next_action=next_action):
            if solution_uuid == self.get_solution_uuid(workload):
