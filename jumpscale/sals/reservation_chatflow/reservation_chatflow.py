...
class Network:
    ...
    def add_node(self, node):
        ...
        for network_resource in network_resources:
            ...
            j.sals.zos.get_zos_for().network.add_node(self._network, node.node_id, str(subnet))
            self._is_dirty = True
    ...
    def update(self, tid, currency=None, bot=None):
        ...
        if self._is_dirty:
            ...
            if payment["free"]:
                ...
                j.sals.zos.get_zos_for().billing.payout_farmers(payment["wallet"], reservation_create)
...
class ReservationChatflow:
    ...
    def show_escrow_qr(self, bot, reservation_create_resp, expiration_provisioning):
        ...
        """
        escrow_info = j.sals.zos.get_zos_for().reservation_escrow_information_with_qrcodes(reservation_create_resp)
    ...
    def _reservation_failed(self, bot, reservation):
        failed = j.sals.zos.get_zos_for().reservation_failed(reservation)
        if failed:
            ...
            link = f"{self._explorer.url}/reservations/{reservation.id}"
            res += f"<h2> <a href={link}>Full reservation info</a></h2>"
            j.sals.zos.get_zos_for().reservation_cancel(reservation.id)
            bot.stop(res, md=True, html=True)
    ...
    def register_reservation(
        ...
        try:
            reservation_create = j.sals.zos.get_zos_for().reservation_register(
                reservation,
                expiration,
    ...
    def list_gateways(self, bot, currency=None):
        ...
        gateways = {}
        farms = {}
        for g in j.sals.zos.get_zos_for()._explorer.gateway.list():
            if not j.sals.zos.get_zos_for().nodes_finder.filter_is_up(g):
                continue
            location = []
            ...
            if farm_id not in farms:
                farms[farm_id] = j.sals.zos.get_zos_for()._explorer.farms.get(farm_id)
    ...
    def list_delegate_domains(self, customer_tid, currency=None):
        ...
        Returns:
            [dict]: [domains names]
        """
        reservations = j.sals.zos.get_zos_for().reservation_list(tid=customer_tid, next_action="DEPLOY")
        domains = dict()
        names = set()
    ...
    def list_networks(self, tid, reservations=None):
        ...
        """
        if not reservations:
            reservations = j.sals.zos.get_zos_for().reservation_list(tid=tid, next_action="DEPLOY")
        networks = dict()
        names = set()
        for reservation in sorted(reservations, key=lambda r: r.id, reverse=True):
            if reservation.next_action != NextAction.DEPLOY:
                    ...
                    while curr_network_resv:
                        if curr_network_resv.metadata:
                            ...
                            if "parent_network" in network_metadata:
                                ...
                                j.sals.zos.get_zos_for().reservation_cancel(parent_resv.id)
                                curr_network_resv = parent_resv
                                continue
                        curr_network_resv = None

                j.sals.zos.get_zos_for().reservation_cancel(solution.rid)
    ...
    def create_network(
        ...
        network = j.sals.zos.get_zos_for().network.create(reservation, ip_range, network_name)
        node_subnets = netaddr.IPNetwork(ip_range).subnet(24)
        network_config = dict()
        use_ipv4 = ip_version == "IPv4"

        j.sals.zos.get_zos_for().network.add_node(network, access_node.node_id, str(next(node_subnets)))
    ...
    def select_farms(self, bot, message=None, currency=None, retry=False, sru=None, cru=None, hru=None, mru=None):
        ...
        farm_names = []
        for f in farms:
            if j.sals.zos.get_zos_for().nodes_finder.filter_farm_currency(f, currency) and self.check_farm_resources(
    ...
    def filter_nodes(self, nodes, free_to_use, ip_version=None):
        ...
        """
        nodes = filter(j.sals.zos.get_zos_for().nodes_finder.filter_is_up, nodes)
        nodes = list(nodes)
        if free_to_use:
            nodes = list(nodes)
            nodes = filter(j.sals.zos.get_zos_for().nodes_finder.filter_is_free_to_use, nodes)
        ...
        if ip_version:
            ...
            if use_ipv4:
                nodefilter = j.sals.zos.get_zos_for().nodes_finder.filter_public_ip4
            else:
                nodefilter = j.sals.zos.get_zos_for().nodes_finder.filter_public_ip6

            nodes = filter(j.sals.zos.get_zos_for().nodes_finder.filter_is_up, filter(nodefilter, nodes))
            if not nodes:
